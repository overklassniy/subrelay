...
// sing-box-extended fork (adds xhttp transport and provider registry).
replace github.com/sagernet/sing-box => github.com/shtorm-7/sing-box-extended v1.13.14-extended-2.5.0

// The extended fork replaces several upstream dependencies with its own
// forks. Go only honors replace directives in the main module, so every
// replace from sing-box-extended's go.mod must be replicated here.
